  ...
  # If faces are detected, iterate through each image and detect landmark points
    ...
    for i in range(0,len(faces)):
      ...
      if (isFirstFrame==True):
        pointsPrev=[]
        pointsDetectedPrev = []
        [pointsPrev.append((p.x, p.y)) for p in landmarkDetector(imDlib, newRect).parts()]
        [pointsDetectedPrev.append((p.x, p.y)) for p in landmarkDetector(imDlib, newRect).parts()]
      ...
      points = []
      pointsDetectedCur = []
      [points.append((p.x, p.y)) for p in landmarkDetector(imDlib, newRect).parts()]
      [pointsDetectedCur.append((p.x, p.y)) for p in landmarkDetector(imDlib, newRect).parts()]
      ...
      if eyeDistanceNotCalculated:
        eyeDistance = interEyeDistance(landmarkDetector(imDlib,newRect).parts())
        print(eyeDistance)
      ...
      for k in range(0,len(landmarkDetector(imDlib, newRect).parts())):
